...
from tests.scripts.match_internal_functions_withdb import get_export_id, get_bid_using_fid, get_bname

def arguments_parser():
    parser = argparse.ArgumentParser(
        prog="script to check performance of symbols_db", description="Checks if stored symbols are matching"
    )
    parser.add_argument(
        "-b",
        "--blint-deep-sbom",
    ...
    if bom_file := args["bom"]:
        ...
        if single:=args["single"]:
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=4) as executor:
                for func_name, bin_names in zip(if_strings, executor.map(get_bnames_ename, if_strings)):
